...
public class DbEntityTest {
	...
	public void testEncodeAsXML() {
		...
		BaseConfigurationNodeVisitor<Object> dummyVisitor = Mockito.mock(BaseConfigurationNodeVisitor.class,
				Mockito.withSettings().defaultAnswer(Mockito.CALLS_REAL_METHODS));
		Mockito.doReturn(null).when(dummyVisitor).visitDbEntity(Mockito.any());
		Mockito.doReturn(null).when(dummyVisitor).visitDbAttribute(Mockito.any());
		Mockito.doReturn(null).when(dummyVisitor).visitDbRelationship(Mockito.any());
		entity.encodeAsXML(encoder, dummyVisitor);
