...
public class DataMapNamespaceTest {
	...
	public void testGetDbEntity() {
		...
		MappingNamespace namespace = Mockito.mock(MappingNamespace.class);
		Mockito.when(namespace.getDbEntity(Mockito.anyString())).thenAnswer(invo -> {
			return dbEntities.get(invo.getArgument(0));
		});
		Mockito.when(namespace.getObjEntity(Mockito.anyString())).thenAnswer(invo -> {
			return objEntities.get(invo.getArgument(0));
		});
		Mockito.when(namespace.getProcedure(Mockito.anyString())).thenAnswer(invo -> {
			return procedures.get(invo.getArgument(0));
		});
		Mockito.when(namespace.getQueryDescriptor(Mockito.anyString())).thenAnswer(invo -> {
			return queryDescriptors.get(invo.getArgument(0));
		});
		Mockito.when(namespace.getDbEntities()).thenAnswer(invo -> {
			return dbEntities.values();
		});
		Mockito.when(namespace.getObjEntities()).thenAnswer(invo -> {
			return objEntities.values();
		});
		Mockito.when(namespace.getProcedures()).thenAnswer(invo -> {
			return procedures.values();
		});
		Mockito.when(namespace.getQueryDescriptors()).thenAnswer(invo -> {
