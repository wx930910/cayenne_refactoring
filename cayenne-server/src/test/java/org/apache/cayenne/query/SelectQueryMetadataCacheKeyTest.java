...
public class SelectQueryMetadataCacheKeyTest {
	...
	public void createObjects() {
		...
		ValueObjectType testType = Mockito.mock(ValueObjectType.class);
		Mockito.when(testType.getTargetType()).thenThrow(new UnsupportedOperationException());
		Mockito.when(testType.getValueType()).thenThrow(new UnsupportedOperationException());
		Mockito.when(testType.toJavaObject(Mockito.anyInt())).thenThrow(new UnsupportedOperationException());
		Mockito.when(testType.fromJavaObject(Mockito.any())).thenThrow(new UnsupportedOperationException());
		Mockito.when(testType.toCacheKey(Mockito.any())).thenAnswer(invocation -> {
			TestValue object = invocation.getArgument(0);
