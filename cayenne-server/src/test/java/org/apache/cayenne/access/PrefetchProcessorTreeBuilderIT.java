...
public class PrefetchProcessorTreeBuilderIT extends ServerCase {
	...
	public void testBuildTreeNoPrefetches() {
		...
		dataRows.add(new DataRow(4));
		QueryMetadata metadata = Mockito.mock(QueryMetadata.class);
		Mockito.when(metadata.getClassDescriptor()).thenAnswer(invo -> {
			return descriptor;
		});
		Mockito.when(metadata.getObjEntity()).thenAnswer(invo -> {
			return descriptor.getEntity();
		});
		Mockito.when(metadata.getDbEntity()).thenAnswer(invo -> {
			return metadata.getObjEntity().getDbEntity();
		});
		Mockito.when(metadata.getDataMap()).thenAnswer(invo -> {
			return metadata.getObjEntity().getDataMap();
		});
		Mockito.when(metadata.isRefreshingObjects()).thenReturn(true);
		Mockito.when(metadata.isRefreshingObjects()).thenReturn(false);
	...
	public void testBuildTreeWithPrefetches() {
		...
		tree.addPath(Artist.PAINTING_ARRAY.dot(Painting.TO_GALLERY).dot(Gallery.EXHIBIT_ARRAY).getName())
				.setPhantom(false);
