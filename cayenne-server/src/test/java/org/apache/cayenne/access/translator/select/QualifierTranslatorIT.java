...
public class QualifierTranslatorIT extends ServerCase {
	...
	private void doExpressionTest(Class<?> queryType, Expression qualifier, String expectedSQL) throws Exception {
		...
		List<DbRelationship> dbRels = new ArrayList<DbRelationship>();

		// TstQueryAssembler qa = new TstQueryAssembler(q, node.getAdapter(),
		// node.getEntityResolver());
		QueryAssembler qa = Mockito.mock(QueryAssembler.class,
				Mockito.withSettings().defaultAnswer(Mockito.CALLS_REAL_METHODS).useConstructor(q, node.getAdapter(),
						node.getEntityResolver()));
		Mockito.doAnswer(invo -> {
			dbRels.add(invo.getArgument(1));
			return null;
		}).when(qa).dbRelationshipAdded(Mockito.any(), Mockito.any(), Mockito.anyString());
		Mockito.when(qa.getCurrentAlias()).thenReturn("ta");
		Mockito.doNothing().when(qa).resetJoinStack();
		Mockito.when(qa.supportsTableAliases()).thenReturn(true);
		Mockito.when(qa.getAliasForExpression(Mockito.any())).thenReturn(null);
		Mockito.doAnswer(invo -> {
			qa.sql = "SELECT * FROM ARTIST";
			return null;
		}).when(qa).doTranslate();
		StringBuilder out = new StringBuilder();
