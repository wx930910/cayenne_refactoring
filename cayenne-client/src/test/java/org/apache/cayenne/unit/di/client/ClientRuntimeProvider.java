...
public class ClientRuntimeProvider implements Provider<ClientRuntime> {
	...
	protected Collection<? extends Module> getModules() {
		return Collections.singleton(binder -> {
			InterceptingClientServerChannelProvider provider = Mockito
					.spy(new InterceptingClientServerChannelProvider(serverRuntimeProvider.get().getInjector()));
			Mockito.when(provider.get()).thenAnswer(invocation -> {
				DataChannel clientServerChannel = (DataChannel) invocation.callRealMethod();
