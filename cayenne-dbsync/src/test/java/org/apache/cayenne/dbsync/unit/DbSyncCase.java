...
public class DbSyncCase extends DICase {
	...
	static {
		...
		Mockito.doAnswer(invo -> {
			...
			binder.bind(ServerRuntime.class).toProvider(DbSyncServerRuntimeProvider.class).in(testScope);
			return null;
		}).when(module).configure(Mockito.any());
		injector = DIBootstrap.createInjector(new ServerCaseModule(testScope), module);
